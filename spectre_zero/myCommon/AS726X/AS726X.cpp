...
byte AS726X::virtualReadRegisterPublic(byte virtualAddr){
	res = virtualReadRegister(virtualAddr);
	return (res);
}
	
byte AS726X::readRegisterPublic(byte virtualAddr){
	res = readRegister(virtualAddr);
	return (res);
}
...
byte AS726X::readRegister(byte addr)
{
	...
	if (_i2cPort->available()) {
		...
		Serial.println("error: i2c readRegister() failed");
		throw std::invalid_argument( "error: i2c readRegister() failed" );
		return (0xFF); //Error
	}
}
